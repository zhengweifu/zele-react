...
var Potrace = (function() {
  ...
  function loadImageFromUrl(url) {
    ...
    if(url.substr(0, 11) == 'data:image/') {
      imgElement.crossOrigin = null; //base64 数据不需要跨域
      // console.log(imgElement.crossOrigin, url);
    } else {
      imgElement.crossOrigin = 'anonymous'; // 可以跨域读取文件
    }
  ...
  function toTHREEPath(path) {
    var curve, n, i, j;
    for(i = 0; i < pathlist.length; i++) {
      curve = pathlist[i].curve;
      n = curve.n;
  ...
  function toFabricPath(fabric) {
    var curve, n, i, j, pathString = '';
    for(i = 0; i < pathlist.length; i++) {
      curve = pathlist[i].curve;
      n = curve.n;
      ...
      pathString += ' ' + curve.c[(n - 1) * 3 + 2].x + ' ' + curve.c[(n - 1) * 3 + 2].y;
  ...
  return {
    ...
    process: process,
    getSVG: getSVG,
    toTHREEPath: toTHREEPath,
    toFabricPath: toFabricPath,
    img: imgElement
  };
})();
